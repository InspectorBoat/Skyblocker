...
public class GuardianHealth {
	...
	private static void extractRendering(PrimitiveCollector collector) {
		...
		if (!Utils.isInDungeons() || DungeonManager.getBoss() != DungeonBoss.PROFESSOR || client.player == null || client.level == null) return;
		List<Guardian> guardians = client.level.getEntitiesOfClass(Guardian.class, bossRoom, _ -> true);
		...
		for (Guardian guardian : guardians) {
			...
			for (ArmorStand armorStand : armorStands) {
				...
				boolean isProfessor = display.contains("The Professor");
				Matcher matcher = isProfessor ? professorRegex.matcher(display) : guardianRegex.matcher(display);
